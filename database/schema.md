...
Debe existir al menos una medición no nula. Las FK usan `ON DELETE RESTRICT`. Índices: `recorded_at DESC`, (`incubator_id`, `recorded_at DESC`), (`device_id`, `recorded_at DESC`) y un índice parcial (`admission_id`, `recorded_at DESC`) donde `admission_id IS NOT NULL`. Una futura política de retención/particionado mensual debe evaluarse antes de producción.
